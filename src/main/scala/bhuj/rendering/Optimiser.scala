...
private[bhuj] class Optimiser(parse: ParseTemplate, templates: Templates = emptyTemplates) {
  ...
  def optimise(template: Template): Either[Failure, Template] = {
    template.components.foldLeft[Either[Failure, Template]](Right(template.copy(components = Seq.empty))) {
      case (Right(acc), partial: Partial) => optimisedPartial(partial).right.map(appendTo(acc))
      case (Right(acc), section: Section) => optimisedSection(section).right.map(section => appendTo(acc)(section))
  ...
  private def optimisedPartial(partial: Partial): Either[Failure, Seq[Component]] = for {
    template    <- partialTemplate(partial).right
    isRecursive <- hasDescendant(template, partial).right
    optimised   <- if (!isRecursive) optimise(template).right else Right(template).right
  } yield {
    if (isRecursive)
      Seq(partial)
    else
  ...
  private def partialTemplate(partial: Partial): Either[Failure, Template] = for {
    rawTemplate <- templates(partial.name).toRight({ TemplateNotFound(partial.name) }).right
    template    <- parse(rawTemplate).right
  } yield { template }
  ...
  private def hasDescendant(template: Template, component: Component): Either[Failure, Boolean] = {
    template.components.foldLeft[Either[Failure, Boolean]](Right(false)) {
      ...
      case (Right(false), c: Partial)          => partialTemplate(c).right.flatMap(hasDescendant(_, component))
      case (Right(false), _)                   => Right(false)
      case (failure: Left[_,_], _)             => failure
    }
  }

  private def optimisedSection(section: Section): Either[Failure, Section] = optimise(section.template).right.map(ot => section.copy(template = ot))
